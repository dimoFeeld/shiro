...
Shiro consists of one project with 3 distinct parts.
...
git clone https://github.com/dimoFeeld/shiro.git
cd Shiro
...
```sh
update-alternatives --install /usr/share/gnome-shell/theme/gdm3.css gdm3.css /usr/share/gnome-shell/theme/Shiro/gnome-shell.css 15
```

If you reboot, select the Shiro session in GDM and you should be all right.

### More granular changes

Now everything should be in place. Select the GTK and sound theme via:
```bash
gsettings set org.gnome.desktop.interface gtk-theme Shiro
gsettings set org.gnome.desktop.sound theme-name Shiro
```

The GTK2 and GTK3 files go into `/usr/local/share/themes/Shiro`. The shell files go into `/usr/local/share/gnome-shell/theme/Shiro`. You can edit the `gtk.css` and `gnome-shell.css` files in those folders directly for testing, or you can edit the SCSS files inside the folder you cloned from GitHub.

SCSS is the actual "source code" of the theme. This is compiled into the CSS files. Edit the SCSS if you want to contribute your changes back to us. SCSS is simple enough to get the hang of if you already know CSS. You can go through [this SCSS tutorial](http://marksheet.io/sass-scss-less.html) to learn more. After making your edits in the SCSS files, you can run `sudo ninja install` in the `Shiro/build` folder. That’ll do all the compiling and installing.
...
```bash
# To reload GTK theme
# Change to Adwaita theme and back to Shiro
gsettings set org.gnome.desktop.interface gtk-theme Adwaita
gsettings set org.gnome.desktop.interface gtk-theme Shiro
# To reload icon theme
# Change to Humanity icon theme and back to Shiro
gsettings set org.gnome.desktop.interface icon-theme Humanity
gsettings set org.gnome.desktop.interface icon-theme Shiro
```
